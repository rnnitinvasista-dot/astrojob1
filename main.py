...
async def job_analysis(req: KundliRequest):
    try:
        ...
        try:
            ...
            return {"status": "error", "message": "Could not find 6th or 10th house sublords."}
        ...
        if not csl6_data or not csl10_data:
            return {"status": "error", "message": f"Missing Nadi data for {csl6_name} or {csl10_name}"}

        prompt_data = {"csl6": csl6_data, "csl10": csl10_data}
        ...
        if "error" in analysis:
            return {"status": "error", "message": f"AI Failed: {analysis['error']}"}
            
        return {"status": "success", "csl_details": prompt_data, "analysis": analysis}
        
    except Exception as e:
        error_msg = traceback.format_exc()
        return JSONResponse(
            status_code=500,
            content={"status": "error", "message": f"Server Crash: {str(e)}", "details": error_msg}
        )
