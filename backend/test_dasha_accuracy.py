...
def verify_dasha():
    ...
    print(f"Current Dasha: {result['dasha']['dasha']} / {result['dasha']['bhukti']} / {result['dasha']['antar_bhukti']} / {result['dasha']['pratyantar']} / {result['dasha']['sukshma']}")
        ...
        if "2026" in md['end_date'] or "1974" in md['start_date'] or "2027" in md['end_date']:
            ...
            for ad in md['bhuktis']:
                if "2026" in ad['end_date'] or "1974" in ad['start_date']:
                    print(f"  BH: {ad['planet']} from {ad['start_date']} to {ad['end_date']}")
                    for pd in ad['antar_bhuktis']:
                         if "2026" in pd['end_date'] or "1974" in pd['start_date']:
                             print(f"    AB: {pd['planet']} from {pd['start_date']} to {pd['end_date']}")
                             for sd in pd['pratyantars']:
                                 if "2026" in sd['end_date']:
                                     print(f"      PR: {sd['planet']} from {sd['start_date']} to {sd['end_date']}")
                                     first_sk = sd['sukshmas'][0]['planet']
                                     print(f"        First SK: {first_sk} (Parent PR: {sd['planet']})")
