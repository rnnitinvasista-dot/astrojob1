...
export const CombinationTab: React.FC<CombinationTabProps> = ({ data, dasha, houses }) => {
    const [selectedCusp, setSelectedCusp] = useState<number>(1);
    const [selectedPlanet, setSelectedPlanet] = useState<string>('Sun');

    useEffect(() => {
        if (!houses) return;
        const targetHouse = houses.find(h => h.house_number === selectedCusp);
        if (targetHouse && targetHouse.sub_lord) {
            const shortL = targetHouse.sub_lord.toLowerCase();
            ...
            let matchedPlanet: string | undefined = mapShortToLong[shortL];
            if (!matchedPlanet) {
                matchedPlanet = ALL_PLANETS.find(p => p.toLowerCase().startsWith(shortL) || shortL.startsWith(p.toLowerCase()));
            }

            if (matchedPlanet && typeof matchedPlanet === 'string') {
        ...
        <div style={{ padding: '16px', background: '#ffffff', borderRadius: '8px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}>
            ...
            {combinations.length === 0 ? (
                ...
                <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch', paddingBottom: '8px' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #e2e8f0', fontSize: '0.8rem' }}>
                        <thead>
                            <tr style={{ background: '#f1f5f9' }}>
                                <th style={{ border: '1px solid #e2e8f0', padding: '10px 6px', fontWeight: 800, textTransform: 'uppercase', color: 'var(--secondary)', width: '15%' }}>ABOUT</th>
                                <th style={{ border: '1px solid #e2e8f0', padding: '10px 6px', fontWeight: 800, textTransform: 'uppercase', color: 'var(--secondary)', width: '18%' }}>CUSP / DASHA REQUIRED</th>
                                <th style={{ border: '1px solid #e2e8f0', padding: '10px 6px', fontWeight: 800, textTransform: 'uppercase', color: 'var(--secondary)', width: '12%' }}>STATUS</th>
                                <th style={{ border: '1px solid #e2e8f0', padding: '10px 6px', fontWeight: 800, textTransform: 'uppercase', color: 'var(--secondary)', width: '10%' }}>D B A</th>
                                <th style={{ border: '1px solid #e2e8f0', padding: '10px 6px', fontWeight: 800, textTransform: 'uppercase', color: 'var(--secondary)', width: '45%' }}>REMARKS</th>
                            </tr>
                        </thead>
                        <tbody>
                            {combinations.map((comb, idx) => {
                                ...
                                return (
                                    <tr key={idx} style={{ background: '#ffffff', transition: 'background 0.2s' }}>
                                        <td style={{ border: '1px solid #e2e8f0', padding: '8px 6px', fontWeight: 700, color: '#1e293b' }}>
                                            {comb.about}
                                        </td>
                                        <td style={{ border: '1px solid #e2e8f0', padding: '8px 6px', fontWeight: 600, color: '#334155' }}>
                                            {comb.cuspText && <div><span style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: 700 }}>CUSP:</span> {comb.cuspText}</div>}
                                            {comb.dashaText && <div style={{ marginTop: comb.cuspText ? '4px' : '0' }}><span style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: 700 }}>DASHA:</span> {comb.dashaText}</div>}
                                        </td>
                                        <td style={{ border: '1px solid #e2e8f0', padding: '8px 6px', textAlign: 'center', fontWeight: 800 }}>
                                            {!comb.cuspText ? (
                                        ...
                                        <td style={{ border: '1px solid #e2e8f0', padding: '8px 6px', textAlign: 'center', fontWeight: 800, fontSize: '0.9rem', letterSpacing: '1px', color: 'var(--primary)' }}>
                                            {dbaMatches.length > 0 ? dbaMatches.join(' ') : <span style={{ color: '#cbd5e1', fontSize: '0.75rem' }}>-</span>}
                                        </td>
                                        <td style={{ border: '1px solid #e2e8f0', padding: '8px 6px', background: (comb.cuspText || comb.dashaText) ? remarkColor : 'transparent', fontWeight: 500, color: '#334155', lineHeight: '1.4', fontSize: '0.75rem' }}>
                                            {comb.remarks || '-'}
                                        </td>
                                    </tr>
                                );
