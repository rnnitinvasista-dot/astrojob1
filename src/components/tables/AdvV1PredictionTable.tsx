...
interface AdvV1PredictionTableProps {
...
const AdvV1PredictionTable: React.FC<AdvV1PredictionTableProps> = ({ 
    data, 
    planets, 
    types, 
    planetName, 
    ...
    // NLS Logic
    const nlDataForV1 = data.find(p => p.planet === planetData.star_lord);
    const nlsSubLord = nlDataForV1?.sub_lord || planetData.sub_lord;
    let nlsHit = placementMap[nlsSubLord.toUpperCase()];

    let plHouses = planetData.pl_signified.map(h => h.house);
    let nlHouses = planetData.nl_signified.map(h => h.house);
    let nlsHouses = nlDataForV1 ? nlDataForV1.sl_signified.map(h => h.house) : planetData.sl_signified.map(h => h.house);

    if (isChildBirth) {
        plHouses = plHouses.filter(h => h !== 3 && h !== 6 && h !== 7);
        nlHouses = nlHouses.filter(h => h !== 3 && h !== 6 && h !== 7);
        nlsHouses = nlsHouses.filter(h => h !== 3 && h !== 6 && h !== 7);
        if (plHit === 3 || plHit === 6 || plHit === 7) plHit = -1;
        if (nlHit === 3 || nlHit === 6 || nlHit === 7) nlHit = -1;
        if (nlsHit === 3 || nlsHit === 6 || nlsHit === 7) nlsHit = -1;
    }
    ...
    const nlsBif = renderHousesPerRow(nlsHouses, nlsHit);

    const comboGoodSet = new Set([...plBif.good, ...nlBif.good, ...nlsBif.good]);
    const comboBadSet = new Set([...plBif.bad, ...nlBif.bad, ...nlsBif.bad]);
    ...
    if (nlsHit !== -1) allHousesSet.add(nlsHit);

    const counts: Record<number, number> = {};
    [...plHouses, ...nlHouses, ...nlsHouses].forEach(h => { counts[h] = (counts[h] || 0) + 1; });
        ...
        return (
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '4px' }}>
                {sorted.map((h, i) => {
                    ...
                    const isCircled = h === nlHit || h === nlsHit;
                    return (
                        <span key={h} style={{
                            color: getHouseColor(h, isBad, selectedArea) === '#ef4444' ? '#ef4444' : getHouseColor(h, isBad, selectedArea) === '#16a34a' ? '#16a34a' : '#2563eb',
                            fontWeight: '800',
    ...
    const successCode = matrix[nlsHit]?.[nlHit] || "M";
    ...
    const plProfessionText = React.useMemo(() => {
        let text = shuffleText((isEducation ? EDU_PROFESSION_MAP : isMarriage ? MARRIAGE_RESULT_MAP : isChildBirth ? CHILD_BIRTH_RESULT_MAP : isHealth ? HEALTH_RESULT_MAP : JOB_PROFESSION_MAP)[plHit]);
        if (isForeignStudies) text += ", Foreign Studies (ACCA, MBA, PMP, CPA, CFA, CMA US, FRM, International Certification)";
        if (isJobAndBusiness) {
            if ([3, 5, 8].includes(plHit)) text += ", Film line";
            if ([8, 12].includes(plHit)) text += ", Spirituality";
        }
        return text;
    }, [plHit, isForeignStudies, isJobAndBusiness]);
    ...
    const slProfessionText = React.useMemo(() => {
        let text = shuffleText((isEducation ? EDU_PROFESSION_MAP : isMarriage ? MARRIAGE_RESULT_MAP : isChildBirth ? CHILD_BIRTH_RESULT_MAP : isHealth ? HEALTH_RESULT_MAP : JOB_PROFESSION_MAP)[nlsHit]);
        if (isForeignStudies) text += ", Foreign Studies (ACCA, MBA, PMP, CPA, CFA, CMA US, FRM, International Certification)";
        if (isJobAndBusiness) {
            if ([3, 5, 8].includes(nlsHit)) text += ", Film line";
            if ([8, 12].includes(nlsHit)) text += ", Spirituality";
        }
        return text;
    }, [nlsHit, isForeignStudies, isJobAndBusiness]);
        ...
        <div style={{
            ...
            <div style={{ padding: '0px' }}>
                {[
                    ...
                    { label: 'NLS', p: nlsSubLord, h: nlsHit, bif: nlsBif }
                ].map((row) => (
                        ...
                        {activeTab === 'DETAILS' ? (
                            <div style={{ padding: '12px' }}>
                                <div style={{ fontSize: '0.8rem', color: '#334155', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                    ...
                                    {nlsHit !== -1 && (
                                        <div style={{ display: 'flex', gap: '6px' }}>
                                            <span style={{ fontWeight: 900, color: '#000000', minWidth: '40px' }}>NLS (60%):</span>
                                            <span>{shuffleText(HEALTH_DISEASE_MAP[nlsSubLord.toUpperCase()] || "General health issues")}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div style={{ padding: '12px' }}>
                                {(() => {
                                    ...
                                    const nlsHasRemedy = nlsHit === 8 || nlsHit === 12;
                                    let targetPlanets: string[] = [];

                                    if (nlHasRemedy && nlsHasRemedy) {
                                        const badSet = new Set([4, 6, 8, 10, 12]);
                                        const nlBadCount = nlBif.bad.filter((h: number) => badSet.has(h)).length;
                                        const nlsBadCount = nlsBif.bad.filter((h: number) => badSet.has(h)).length;
                                        
                                        if (nlBadCount > nlsBadCount) {
                                            targetPlanets.push(planetData.star_lord);
                                        } else if (nlsBadCount > nlBadCount) {
                                            targetPlanets.push(nlsSubLord);
                                        } else {
                                            targetPlanets.push(planetData.star_lord);
                                            targetPlanets.push(nlsSubLord);
                                        }
                ...
                {isExpanded && !isTransitMode && !isHealth && (
                    <div style={{ background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
                        ...
                        {activeTab === 'DETAILS' ? (
                            <div style={{ padding: '16px' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                    {(isEducation || isChildBirth || selectedArea === 'Job' || selectedArea === 'Business') && (
                                        <div style={{ display: 'flex', gap: '8px', fontSize: '0.85rem' }}>
                                            <span style={{ fontWeight: 900, color: '#35a4f4' }}>PL:</span>
                                            <span style={{ color: '#334155' }}>{plProfessionText}</span>
                                        </div>
                                    )}
                                    <div style={{ display: 'flex', gap: '8px', fontSize: '0.85rem' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '8px', fontSize: '0.85rem' }}>
                                        <span style={{ fontWeight: 900, color: '#35a4f4' }}>NLS{(selectedArea !== 'Travel' && selectedArea !== 'Property & Vehicle' && selectedArea !== 'Child Birth') ? ' (60%)' : ''}:</span>
...
export default AdvV1PredictionTable;
